...
export default function ChatBoxSendForm({ onSend }: ChatBoxSendFormProps) {
  ...
  return (
    <div className="sticky bottom-0 p-3 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
      <form onSubmit={handleSubmit} className="flex items-center justify-between gap-3">
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full pl-12 pr-5 text-sm text-gray-800 bg-transparent border-none outline-hidden h-11 placeholder:text-gray-400 focus:border-0 focus:ring-0 dark:text-white/90"
          />
        </div>
        ...
        <div className="flex items-center">
          ...
          <button 
            ...
            className="flex items-center justify-center ml-3 text-white rounded-lg h-10 w-10 bg-brand-500 hover:bg-brand-600 disabled:opacity-50 disabled:cursor-not-allowed xl:ml-5 transition-colors"
          >
            <svg
              width="20"
              height="20"
      ...
      <div className="flex items-center gap-2 mt-2 pt-2 border-t border-gray-100 dark:border-gray-800">
        <span className="text-xs text-gray-400 dark:text-gray-500">Быстрые ответы:</span>
        <button 
          type="button"
          onClick={() => onSend("Автомобиль готов к выдаче!")}
          className="px-2 py-1 text-xs text-gray-600 bg-gray-100 rounded-full hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700"
        >
          Машина готова
        </button>
        <button 
          type="button"
          onClick={() => onSend("Перезвоню в ближайшее время")}
          className="px-2 py-1 text-xs text-gray-600 bg-gray-100 rounded-full hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700"
        >
          Перезвоню
        </button>
        <button 
          type="button"
          onClick={() => onSend("Спасибо, ждем вас снова!")}
          className="hidden sm:block px-2 py-1 text-xs text-gray-600 bg-gray-100 rounded-full hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700"
        >
          Спасибо
        </button>
      </div>
